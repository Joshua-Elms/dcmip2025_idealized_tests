...
class Inference(torch.nn.Module, time_loop.TimeLoop):
    ...
    def __call__(
        ...
        x: torch.Tensor,
        recurrent_perturbation: Optional[torch.Tensor] = None,
        ...
        Args:
            ...
            recurrent_perturbation: a tensor with shape (B, n_history_levels,
                len(in_channel_names), Y, X) which is added to the model output
                at each time step. This is useful for the H&M24 "tropical 
                heating" experiment. 
        ...
        if restart:
            ...
            yield from self._iterate(x=x, time=time, recurrent_perturbation=recurrent_perturbation)

    def _iterate(self, x, normalize=True, time=None, recurrent_perturbation=None):
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                x = self.model(x, time)
                # add some perturbation *after* each timestep, account for normalization
                if recurrent_perturbation is not None:
                    if normalize:
                        x_with_units = x * self.scale + self.center
                        x = ((x_with_units+recurrent_perturbation) - self.center) / self.scale
                        
                    else:
                        x += recurrent_perturbation
