...
for model_name in models:
    ...
    # tendency_file = output_dir / "auxiliary" / f"tendency_{model_name}.nc" # unneeded for current vis, not present under TR=False
    ...
    if ds.sizes["lead_time"] >= 16:
        ...
        for it in [0, 48, 72, 96]:  # lead times in hours
            ...
            z500_pert = (
                ds["z500"].sel(lead_time=np.timedelta64(it, "h")).squeeze().values
            )
            u500_pert = (
                ds["u500"].sel(lead_time=np.timedelta64(it, "h")).squeeze().values
            )
            v500_pert = (
                ds["v500"].sel(lead_time=np.timedelta64(it, "h")).squeeze().values
            )
            ...
            # colorize land
            ax[axi].add_feature(
                cfeature.LAND, edgecolor="0.5", linewidth=0.5, zorder=-1
            )
            ...
            ax[axi].text(
                130, 20, f"{panel_label[axi]}:{it}hrs", transform=ccrs.PlateCarree()
            )

        fig.tight_layout()
        plt.savefig(
            plot_dir / f"IVP_500_{model_name}.pdf", dpi=300, bbox_inches="tight"
        )
    ...
    n_timesteps = 4
    titles = [
        f"{model_name.upper()}: $Z_{{500}}$ Anomalies from {season} Climatology at t={t*6} hours"
