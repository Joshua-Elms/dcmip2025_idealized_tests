...
if model_name not in models:
    print(f"Model {model_name} not in config models list, skipping visualization.")
    exit()
...
for it in [0,2,3,4]:
    ...
    z500_pert = ds["z500"].isel(lead_time=it).squeeze().values
    u500_pert = ds["u500"].isel(lead_time=it).squeeze().values
    v500_pert = ds["v500"].isel(lead_time=it).squeeze().values
    ...
    ax[axi].text(130,20,f"{panel_label[axi]}:{it*24}hrs",transform=ccrs.PlateCarree())
...
titles = [f"{model_name.upper()}: $Z_{{500}}$ Anomalies from {season} Climatology at t={t*24} hours" for t in range(0, n_timesteps+1)]
...
titles = [f"{model_name.upper()}: $T_{{500}}$ Anomalies from {season} Climatology at t={t*24} hours" for t in range(0, n_timesteps+1)]
