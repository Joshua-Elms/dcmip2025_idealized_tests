...
standardized_ylims = (1010, 1019) # y-limits for the plot, set to None to use the model output min/max
...
### Loop through each model and plot the mean results ###
assert plot_var == "MSL", "Plotting MSLP only, please set plot_var to 'MSL'"
title = f"Model Forecasted MSLP vs. Expected Response"
save_title = f"GRC_MSLP_conservation.png"
ylab = "MSLP (hPa)"
linewidth = 4
fontsize = 26
smallsize = 22
linestyle = "solid" # see https://matplotlib.org/stable/gallery/lines_bars_and_markers/linestyles.html
cmap = colormaps.get_cmap(cmap_str)
qual_colors = cmap(np.linspace(0, 1, n_ics))
model_names = {
    "sfno": "FourCastNetv2",
    "graphcast_oper": "GraphCast",
    "pangu": "Pangu-Weather",
}
model_colors = {
    "sfno": "xkcd:green",
    "graphcast_oper": "xkcd:blue",
    "pangu": "xkcd:orange",
}
fig, ax = plt.subplots(figsize=(12.5, 6.6))
ax.plot(lead_times, e5_pds[f"{plot_var}_mean"], color="black", linewidth=linewidth, label="ERA5 mean (expected result)", linestyle=linestyle)
...
for model in models:
    ...
    ax.plot(lead_times, ens_mean, color=model_colors[model], linewidth=linewidth, label=f"{model_names[model]} mean", linestyle=linestyle)
...
ax.set_yticks(yticks, yticks.astype(int), fontsize=smallsize)
...
fig.tight_layout()
plt.legend(fontsize=21, loc="upper left", ncols=1)
