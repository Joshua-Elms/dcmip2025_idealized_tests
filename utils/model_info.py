...
PL_VARIABLES = [
    f"{var}{level}"
    for var in ["u", "v", "t", "q", "r", "w", "z"]
    for level in STANDARD_13_LEVELS
]
IN_VARIABLES = ["z", "lsm"]

MASTER_VARIABLES_NAMES = SL_VARIABLES + PL_VARIABLES + IN_VARIABLES
MASTER_VARIABLES_TYPES = (
    [SL for _ in SL_VARIABLES] + [PL for _ in PL_VARIABLES] + [IN for _ in IN_VARIABLES]
)
...
MODEL_VARIABLES = dict(
    ...
    FCN=dict(
        ...
        types=[SL for _ in range(5)]
        + [PL for _ in range(14)]
        + [SL for _ in range(3)]
        + [PL for _ in range(4)],
    ),
    FuXi=dict(
...
# add moisture var indices using simple code
for model_name in MODEL_VARIABLES.keys():
    names = MODEL_VARIABLES[model_name]["names"]
    moisture_vars = [var_name in MOISTURE_VARIABLE_NAMES for var_name in names]
    MODEL_VARIABLES[model_name]["moist"] = moisture_vars
