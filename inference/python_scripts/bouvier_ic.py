...
# read configuration
config_path = Path('/glade/u/home/jmelms/projects/dcmip2025_idealized_tests/initial_conditions/bouvier/configs/long.yml')
...
for i, val in enumerate(iter_vals.tolist()): # whichever parameter is iterable
    ...
    ic = process_individual_fort_file(**nc_kwargs)
    
    # put the initial condition into a format compatible with the SFNO
    x = dcmip.pack_sfno_state(ic, device=device)

    logging.info(f"Loaded IC-{i}: iter_param={val}")

    # run the model
    data_list = []
    iterator = model(dt.datetime(1850,1,1), x)
    for k, (time, data, _) in enumerate(iterator):
        logging.info(f"Step {k+1} completed.")
        data_list.append(data.to(device="cpu"))
        if k == config["inference_parameters"]["n_steps"] - 1:
            break
    ...
    if i == 0:
        ...
        ds_out = xr.concat([ds_out, ds], dim=iter_param)
...
logging.info(f"Saved ds of size {ds.nbytes/1e9:.2f} GB to {output_path}")
